...
const INJECTION_PATTERNS = [
  ...
  },
  {
    name: "tool_abuse",
    pattern:
      /(?:\b(?:delete\s+all\s+files|wipe\s+the\s+disk|format\s+drive|destroy\s+all\s+records|del\s+\/[sS]\s+\/[qQ]|format\s+[a-z]:|dd\s+if=\/dev\/zero|mkfs\.|drop\s+(?:database|table|schema)\s+[a-zA-Z0-9_*]+|truncate\s+table\s+[a-zA-Z0-9_*]+)\b|\brm\s+-rf\s+[/~*])/i,
    severity: "high",
  },
  {
    name: "credential_theft",
    pattern:
      /\b(?:give\s+me|tell\s+me|send\s+me|provide|extract|show|dump|print|read|cat)\b[\s\S]{0,80}?(?:\b(?:api\s*keys?|database\s+passwords?|db\s+passwords?|admin\s+credentials?|secret\s+keys?|private\s+keys?|master\s+passwords?|id_rsa|id_ed25519|credentials\.json)\b|\.env|\/etc\/shadow|\/etc\/passwd)/i,
    severity: "high",
  },
  {
    name: "context_poisoning",
    pattern:
      /(?:\[\s*(?:SYSTEM(?:\s+NOTE)?|ADMIN(?:\s+OVERRIDE)?|SECURITY(?:\s+COMPLIANCE)?)\s*\]|\[SYSTEM\s+NOTE:[^\]]+\]|\b(?:authentication\s+token\s+is\s+now\s+exempt|security\s+compliance\s+check\s+completed[.,\s]+disable\s+safety)\b)/i,
    severity: "high",
  },
  {
    name: "indirect_injection",
    pattern:
      /(?:!\[.*?\]\(https?:\/\/[^\s)]+?[?&](?:exfil|leak|token|key|data)=|<!--\s*(?:system|override|admin|inject):[\s\S]{0,200}?-->)/i,
    severity: "medium",
  },
  {
    name: "multi_step_jailbreak",
    pattern:
      /\b(?:in\s+this\s+hypothetical\s+story|for\s+the\s+sake\s+of\s+fiction|assume\s+an\s+alternate\s+universe\s+where)\b[\s\S]{0,120}?\b(?:you\s+have\s+no\s+(?:rules|filters|morals)|no\s+safety\s+restrictions|ignore\s+ethical\s+constraints)\b/i,
    severity: "medium",
  },
];
